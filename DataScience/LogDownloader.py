        ...
        if create_gzip_mode > -1:
            if selected_fps:
                ...
                if create_gzip_mode == 0:
                    ...
                    downloaded_fps.sort(key=lambda x : list(map(int,x.split('_data_')[1].split('_')[:3]))+[x])
                    start_date = '-'.join(downloaded_fps[0].split('_data_')[1].split('_')[:3])
                    end_date = '-'.join(downloaded_fps[-1].split('_data_')[1].split('_')[:3])
